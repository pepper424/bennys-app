/* Bennys logic engine - pure functions, no DOM, no network.
...
(function (root, factory) {
  ...
  function isExpiringSoon(b, now) {
    ...
    return d !== null && d >= 0 && d <= EXPIRE_SOON_DAYS;
  }
  ...
  function fmtValue(v) {
    if (!v) return null;
    if (v === Math.round(v)) return "$" + Math.round(v).toLocaleString("en-US");
